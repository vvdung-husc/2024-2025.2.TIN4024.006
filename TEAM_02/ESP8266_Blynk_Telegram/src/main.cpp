...
String checkTemperatureThreshold(float temp) {
  if (temp < 10) return "Nguy cơ suy giảm miễn dịch";
  if (temp >= 10 && temp < 15) return "Tăng nguy cơ bệnh hô hấp";
  if (temp >= 25 && temp <= 30) return "Nhiệt độ lý tưởng";
  if (temp > 35 && temp <= 40) return "Nguy cơ sốc nhiệt";
  if (temp > 40) return "CỰC KỲ NGUY HIỂM";
  return "Bình thường";
}

String checkHumidityThreshold(float humidity) {
  if (humidity < 30) return "Da khô, kích ứng";
  if (humidity >= 40 && humidity <= 60) return "Mức lý tưởng";
  if (humidity > 70 && humidity <= 80) return "Tăng nguy cơ nấm mốc";
  if (humidity > 80) return "Khó thở, nguy cơ sốc nhiệt";
  return "Bình thường";
}
...
void blinkYellowLight() {
  ...
  if (currentTime - lastYellowToggle >= 500) {  // Nhấp nháy 2 lần/giây
