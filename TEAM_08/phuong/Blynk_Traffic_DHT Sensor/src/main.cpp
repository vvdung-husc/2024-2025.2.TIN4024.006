...
#define BLYNK_TEMPLATE_ID "TMPL6TdsfEoyc"
#define BLYNK_TEMPLATE_NAME "ESP32 LED TM1637"
#define BLYNK_AUTH_TOKEN "GK21oBaGdSuEPu-mqgfpRE8JyzVmX28_"
...
#define DHTPIN 16    // Pin where DHT22 data is connected
...
void loop() {  
  ...
  uptimeBlynk();
  updateBlueButton();
  readAndDisplayTemperature(); // Read and display temperature/ humidity
}
...
void readAndDisplayTemperature() {
  ...
  // Gửi nhiệt độ lên chân ảo V2
  Blynk.virtualWrite(V2, temperature); // Gửi giá trị nhiệt độ vào V2

  // Gửi độ ẩm lên chân ảo V3
  Blynk.virtualWrite(V3, humidity); // Gửi giá trị độ ẩm vào V3

  // Hiển thị nhiệt độ trên TM1637 nếu đèn xanh bật
  if (blueButtonON) {
    display.showNumberDec((int)temperature); // Hiển thị nhiệt độ dưới dạng số nguyên
  }
}
